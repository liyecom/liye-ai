# LiYe Systems Architecture v1.3
...
2026-08-23 复评窗结束前不得提前撤销 L1。复评须区分：`(a)` in-scope 事件发生但被 ad-hoc 处理，表示需求已被证伪，复活须击败在位 ad-hoc 模式；或 `(b)` in-scope 事件未发生，表示窗口信息量低，但事件稀薄本身不支持常驻层，第一个真实事件才触发复活评审。若届时仍零自然调用，则由 operator 在复评日另行翻牌，撤销 mandatory L1、改为“L1 可选、按需出现”，并先收割、后定级封存；若出现真实 cross-domain 案例，Loamwise 的保留或复活也必须 conform execution contract。未来 runner 不自动继承 L1 席位。

### Layer 2: Domain Engines — 专业执行器
在特定领域产出 domain truth。遵守 engine_manifest 与治理合约，各自实现执行面。D2 是执行面的性质：conform 于 `_meta/contracts/execution/`；不以某个 runner、编排层席位或 dispatch 路径定义。
...
### Governance（治理/合约）
```
liye_os → loamwise               条件式合约 consumer；若运行也须 conform execution contract
liye_os → domain engines         engine_manifest + execution contract 约束
```

### Runtime（运行时/调度）
```
loamwise → domain engines        条件式 in-scope 路径；非 D2 前置、非 AGE 当前默认执行路径
...
| governed execution contract | 已声明 conform 的执行面；Loamwise 如已绑定也在内 |
...
| D2 | Execution-conformant | 执行面绑定并 conform `_meta/contracts/execution/`；不要求 Loamwise 或其他特定 runner |
...
**D1 → D2 (Execution-conformant):**
- [ ] 执行面声明并 pin `_meta/contracts/execution/governed_execution_v0.schema.yaml` 的兼容版本
- [ ] engine-owned contract tests 覆盖实际执行路径产出的 `per_run` 文档，并验证 schema 与语义约束
- [ ] 提供真实 binding 证据（import / registration / call chain）和至少一份 receipt/readback；只有 resolver 或 fixture 不算绑定
- [ ] live 路径另证既有写信封、被调用方刹车、receipt 与 readback；conformance 本身不授予写权限

当前 execution contract 的诚实状态仍是 `schema_validated_only`。合约包存在不等于任何 engine 已绑定，也不触发 AGE / UGE 正式成熟度标签变更；promotion rule 仍须单独成文。
...
| Loamwise L1 | D2 已去 Loamwise 化；L1 已收窄、非 AGE 默认路径，2026-08-23 复评未到期 | 不提前收窗；按 `(a)` ad-hoc 或 `(b)` 无事件记录零调用类型，再裁定席位与收割/封存 |
