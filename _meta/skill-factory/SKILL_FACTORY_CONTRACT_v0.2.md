...
- 单一 SSOT parser：`.claude/scripts/lib/sfc_frontmatter.mjs`（用 js-yaml 真解析，非正则），导出 `parseFrontmatter()` 与 `checkCompliance()`；sfc_lint / sfc_ci_gate / sfc_sweep 全部经它判定，不再各自硬编码 REQUIRED_KEYS。
