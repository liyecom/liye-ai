...
**Commit anchor**: 对齐 ADR-GHL 教条「the post-squash merge commit becomes the durable anchor; no pre-squash SHA is normative」。**Accept≠merge**：Accept 时 anchor=pending（承载本 ADR 的 liye_os schema/ADR PR 仍 open，frontmatter 不预编 SHA）；PR squash-merge 后回填 post-squash merge SHA 为 durable anchor（docs-only follow-up）。**已回填**：承载 PR #170 于 2026-06-14 由 `liyecom` squash-merge → post-squash merge commit **`73e4d00`**（full `73e4d0072e5dab6bfd4faa0126f8a622c4702e9d`）= 本 ADR durable commit anchor（亦即 AGE re-vendor PR #455 的 freshness `EXPECTED_LIYE_OS_SOURCE_COMMIT`）。
