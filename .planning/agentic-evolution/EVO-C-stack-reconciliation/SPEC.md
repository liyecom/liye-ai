...
**Status**: **v1.0**（ceremony passed；EVO-C IMPL **未启动**，须 operator 明示 go）
...
| proactive kill_switch（EXECUTE_LIMITED_ENABLED） | **新增** `tests/governance/test_proactive_kill_switch.mjs`，断言 default-disabled + ENV-precedence-over-config（`isExecuteLimitedEnabled`）+ `execute_limited_gate` 消费 `isExecuteLimitedEnabled()`（纯断既存行为，零生产改动，test-scope ∈ D-A6/Hard Gate 4） | **本 PR wire 为 BLOCKING CI step（无 `\|\| true`）**——实扫确认 `generate_pr_evidence.mjs` 的 kill-switch self-check **当前无 CI 调用**、`contracts-gate.yml` 只跑 contracts validator 不跑 proactive 行为测试 → **不留 self-check 充数口**，必须新增并接入 blocking CI |
...
- [ ] **D-7** write_executor + 三 kill_switch 字节级回归（per-impl 证据表三 leg 全绿）；其中 **proactive kill_switch 必须新增 `tests/governance/test_proactive_kill_switch.mjs` 并 wire 为 BLOCKING CI（无 `\|\| true`）**——禁以 generate_pr_evidence self-check 充数（无 CI 调用）
- [ ] **DEP-03**（DEFER）：`policy_lifecycle_event_v1.schema.yaml:70` prose「(e.g. system:tier_manager)」标 known non-consumer（description 块内 prose，非 :66 pattern）。**validation-neutral**：`pattern`/`required`/`enum` 不变 + `validate-contracts` schemaFiles count 维持 **21**。⚠ 改 description prose → 文件 hash 必变，**不声称 sealed hash 不变**
