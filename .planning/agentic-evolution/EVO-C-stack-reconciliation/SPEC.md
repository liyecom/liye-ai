...
- D-2: **2 处 kill_switch + execution_gate** 文件头语义重分类注释（enforcement primitive, not learning lifecycle）——`src/governance/learning/execution_gate.mjs` + `src/governance/learning/kill_switch.mjs` + `.claude/scripts/proactive/kill_switch.mjs`（共享同一 state file 的第二实现，消费者 execute_limited_gate/generate_pr_evidence，D-A1）。
...
- [ ] D-1 4 文件 superseded 标记 + D-2 3 文件重分类注释落地，diff 仅注释行（diff 实证）
