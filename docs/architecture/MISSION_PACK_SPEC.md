...
│   ├── summary.md    # Auto-generated summary (≤10 lines, rebuildable)
│   └── ...           # Additional outputs
...
| `needs_manual` | Broker unavailable, manual completion required |
...
| `summary.md` | Auto-generated summary (≤10 lines) - see below |
...
When `liye mission ingest` runs and `answer.md` exists, a deterministic summary is generated:

**Format:**
```markdown
> Auto-summary (deterministic) · Generated at <ISO-timestamp>

<up to 10 lines extracted from answer.md>
```

**Generation Rules:**
1. Max 10 lines, each ≤120 characters (truncated with `…`)
2. Prioritizes content from sections titled: 结论 | 总结 | 建议 | 决策 | 下一步 | conclusion | summary | recommendation
3. Falls back to first N non-empty lines from answer.md
4. Headers and code blocks are skipped

**Rebuilding:**
summary.md is a derived artifact and can always be regenerated:
```bash
