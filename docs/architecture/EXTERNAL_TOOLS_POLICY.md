...
* CrewAI（Python 包 / CLI）
* Claude Code Skills（如 `/crewai`；`/skill-forge` 已于 2026-06-27 退役）
* Anthropic 官方技能库
...
### 4.2 ~~`/skill-forge` 技能~~（已退役 2026-06-27）

> ⚠️ **已退役**：第三方 Skill Forge 已退役并删除，技能创建改用官方 skill-creator（`/create-skill`）。
> provenance 见 `_meta/skill-factory/retired-skills/skill-forge.yaml`。其"从外部来源 ingest 源材料"
> 的能力将由受治理工具 `tools/source-intake/` 重新实现（见 source-intake SPEC，规划中）。
> 下列原则**继续适用于该替代工具与官方 skill-creator**：
