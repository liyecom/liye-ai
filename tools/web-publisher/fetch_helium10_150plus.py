...
def clean_content(text: str) -> str:
    ...
    end_markers = [
        ...
        'Helium 10 Tools', 'Start Your Free Trial', 'Never miss insights'
    ]
...
def fetch_article_list(page, page_num: int = 1, category: str = None):
    """获取文章列表"""
    if category:
        if page_num == 1:
            url = f"https://www.helium10.com/blog/category/{category}/"
        else:
            url = f"https://www.helium10.com/blog/category/{category}/page/{page_num}/"
    else:
        if page_num == 1:
...
def main():
    print(f"[Helium 10 150+ Expansion] Target: {TARGET_MIN}-{TARGET_MAX} articles\n")

    output_dir = Path.home() / 'data/T2_raw/helium10'
    output_dir.mkdir(parents=True, exist_ok=True)

    # 获取已存在的文件
    existing_files = set(f.stem for f in output_dir.glob("*.md") if not f.name.startswith("_"))
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for category in CATEGORIES:
                ...
                for page_num in range(1, 6):  # 每个分类最多5页
                    articles = fetch_article_list(page, page_num, category)
                    ...
                    print(f"    Page {page_num}: +{new_count} new, Total: {len(all_articles)}")
            ...
            for i, article in enumerate(all_articles, 1):
                # 检查是否已存在
                filename = re.sub(r'[^\w\s-]', '', article['title'])
                filename = re.sub(r'[-\s]+', '-', filename)[:60]
    ...
    # 更新统计（包含已有文件）
    final_count = len(list(output_dir.glob("*.md"))) - 1  # 减去_stats.json
    stats['final_total'] = final_count
    ...
    print(f"\n{'=' * 50}")
    print(f"=== Helium 10 150+ Complete ===")
    print(f"New articles: {stats['success_count']}")
