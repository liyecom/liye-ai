...
def clean_content(text: str) -> str:
    ...
    end_markers = [
        'Subscribe to our newsletter', 'Related articles', 'Share this post',
        'Leave a comment', 'About the author', 'Start your Amazon business',
        'Get started with Jungle Scout', 'Sign up for free', 'Footer',
        '© 2025 Jungle Scout', 'Privacy Policy', 'Terms of Service',
...
def fetch_article_content(page, url: str):
    try:
        ...
        selectors = ['article', '.post-content', '.entry-content', 'main article']
        content = ""
...
def fetch_article_list(page, page_num: int = 1):
    ...
    try:
        ...
        articles = []
        links = page.locator('a[href*="/resources/articles/"]').all()

        for link in links:
            try:
                href = link.get_attribute('href')
                if not href or '/resources/articles/' not in href:
                    continue
...
def main():
    print(f"[Jungle Scout Expanded] Target: {TARGET_MIN}-{TARGET_MAX} articles\n")

    output_dir = Path.home() / 'data/T2_raw/junglescout'
